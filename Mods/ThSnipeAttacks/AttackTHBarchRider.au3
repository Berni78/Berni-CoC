#cs ----------------------------------------------------------------------------
    ...
    Author:   shariqriaz  ( Modified By The Master )

    Script Function: AttackTHBarchRider
...
Func AttackTHBarchRider()

SetLog($thinfo)
Setlog("Sniping TH with BarchRider",0x9e3f1b)
...
AttackTHGridMod($eHogs, 2, 1, 1240, 1, 2, 0) ;deploys 2 Hogs in case of spring traps
    If CheckOneStar() Then Return
AttackTHGridMod($eHogs, 2, Random(3, 4, 1), 1500, 2, 2, 0) ;deploys 6-8 Hogs to take heat
	If CheckOneStar() Then Return
SpellTHGrid($eHSpell)
    If CheckOneStar() Then Return
...
AttackTHGridMod($eHogs, 2, 9, 1500, 3, 2, 0) ;deploys up to 18 Hogs
	If CheckOneStar() Then Return
SpellTHGrid($eHSpell)
    If CheckOneStar() Then Return
