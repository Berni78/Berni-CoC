...
Func checkDefense()
	...
	Local $defSimilarity[5]=[0.930,0.940,0.930,0.930,0.930]
	...
	For $t=0 to 4
		For $i = 0 To 9
			If FileExists($trapTH[$t][$i]) Then
				...
				$res = DllCall($LibDir & "\ImageSearch.dll", "str", "searchTile", "handle", $sendHBitmap, "str", $trapTH[$t][$i], "float", $defSimilarity[$t])
					...
					Else
						...
						If $chkInferno = 1 AND $DefText[$t] = "Inferno Tower" Then														
							If ($Defx > 40 AND $Defx < 210) AND ($Defy > 30 AND $Defy < 150) Then
								$skipBase = True
								Return "Inferno Tower found near TH, skipping..."&$i
							Else
								$skipBase = False
							EndIf
						ElseIf $chkTesla = 1 AND $DefText[$t] = "Hidden Tesla" Then														
							If ($Defx > 58 AND $Defx < 192) AND ($Defy > 45 AND $Defy < 135) Then
								$skipBase = True
								Return "Hidden Tesla found near TH, skipping..."&$i
							Else
								$skipBase = False
							EndIf
						ElseIf $chkMortar = 1 AND $DefText[$t] = "Mortar" Then							
							If ($Defx > 5 AND $Defx < 245) AND ($Defy > 10 AND $Defy < 170) Then
								$skipBase = True
								Return "Mortar found near TH, skipping..."&$i
							Else
								$skipBase = False
							EndIf
						ElseIf $chkWiz = 1 AND $DefText[$t] = "Wizard Tower" Then							
							If ($Defx > 53 AND $Defx < 197) AND ($Defy > 42 AND $Defy < 138) Then
								$skipBase = True
								Return "Wizard Tower found near TH, skipping..."&$i
							Else
								$skipBase = False
							EndIf
						ElseIf $chkAir = 1 AND $DefText[$t] = "Air Defense" Then							
							If ($Defx > 15 AND $Defx < 235) AND ($Defy > 20 AND $Defy < 160) Then
								$skipBase = True
								Return "Air Defense found near TH, skipping..."&$i
							Else
								$skipBase = False
							EndIf
						Else							
