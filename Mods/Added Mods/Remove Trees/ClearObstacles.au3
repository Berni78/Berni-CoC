...
GUICtrlSetData ($chkTombstones,"Clear Tomb/Trees")
...
$CoRunCounter +=1
	...
	$obstacles[0] = @ScriptDir & "\Mods\images\obstacles\" & string($ObsFolder) & "\gem.bmp"
	$obstacles[1] = @ScriptDir & "\Mods\images\obstacles\" & string($ObsFolder) & "\bush.bmp"
	$obstacles[2] = @ScriptDir & "\Mods\images\obstacles\" & string($ObsFolder) & "\tree.bmp"
	$obstacles[3] = @ScriptDir & "\Mods\images\obstacles\" & string($ObsFolder) & "\mtree.bmp"
	$obstacles[4] = @ScriptDir & "\Mods\images\obstacles\" & string($ObsFolder) & "\btree.bmp"
	$obstacles[5] = @ScriptDir & "\Mods\images\obstacles\" & string($ObsFolder) & "\vtrunk.bmp"
	$obstacles[6] = @ScriptDir & "\Mods\images\obstacles\" & string($ObsFolder) & "\htrunk.bmp"
	$obstacles[7] = @ScriptDir & "\Mods\images\obstacles\" & string($ObsFolder) & "\mushroom.bmp"
	$obstacles[8] = @ScriptDir & "\Mods\images\obstacles\" & string($ObsFolder) & "\Gembox.bmp"
	$obstacles[9] = @ScriptDir & "\Mods\images\obstacles\" & string($ObsFolder) & "\Tree2.bmp"
	...
	For $obsCnt = 0 To 9
		...
		$res = DllCall(@ScriptDir & "\Mods\lib\ImageSearch.dll", "str", "searchTile", "handle", $sendHBitmap, "str", $obstacles[$obsCnt], "float", 0.910)
