...
If GUICtrlRead($chkSwitchD) = $GUI_CHECKED Then
		IniWrite($config, "advanced", "sw", 1)
	Else
		IniWrite($config, "advanced", "sw", 0)
	EndIf
	...
	Else
		IniWrite($config, "advanced", "SpellNone", 0)
	Endif

	IniWrite($config, "advance", "spelltype", _GUICtrlComboBox_GetCurSel($cmbTHSpellType))
