...
;Local $hBMP= _GDIPlus_BitmapCreateFromHBITMAP($hBitmap)
...
if  Not FileExists(@ScriptDir & "\images\main.bmp") Then
	...
	;_GDIPlus_ImageDispose($hBitmap)
Elseif  Not FileExists(@ScriptDir & "\images\mini.bmp") Then
	 _GDIPlus_ImageSaveToFile($hBMP_Cropped, @ScriptDir & "\images\mini.bmp")
	;_GDIPlus_ImageDispose($hBitmap)
EndIf
;_GDIPlus_ImageDispose($hBitmap)
...
$bm3 = _GDIPlus_ImageLoadFromFile (@ScriptDir & "\images\mini.bmp")
...
If CompareBitmaps($bm1, $bm2) =  True Then
  ...
  ; $building =(@ScriptDir & "\mainb.ini")
   ElseIf CompareBitmaps($bm3, $bm2) =  True Then
       SetLog("Mini account Detected...", $COLOR_Blue)
   ...
   cmbProfile()
	 ...
	 Else
		...
		;$building =(@ScriptDir & "\building.ini")
	 EndIf
